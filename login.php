    ...
    <div class="login-wrapper">
        ...
        <div class="login-card">
            ...
            <div class="hint-card">
                ...
                <div class="hint-row">
                    <span>Password</span>
                    <code id="hintPass" onclick="fillCred('password','admin123')" title="Click to fill">admin123</code>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        if (alreadyIn) {
            ...
            banner.style.cssText =
                ...
                + 'padding:12px 18px;margin-bottom:18px;font-size:0.83rem;'
                + 'display:flex;align-items:center;justify-content:space-between;gap:10px;';
            banner.innerHTML =
                '<span style="color:#2e7d32;font-weight:600;">✅ You are already logged in.</span>'
                + '<div style="display:flex;gap:8px;">'
                + '<a href="staff_dashboard.php" style="background:var(--accent-orange);color:#fff;'
                + 'padding:5px 14px;border-radius:6px;text-decoration:none;font-size:0.8rem;font-weight:700;">Go to Dashboard →</a>'
                + '<button onclick="sessionStorage.removeItem(\'ros_auth\'); this.closest(\'div\').parentNode.remove();" '
                + 'style="background:#ccc;color:#333;border:none;padding:5px 12px;border-radius:6px;font-size:0.8rem;cursor:pointer;">Logout</button>'
                + '</div>';
            document.getElementById('loginForm').insertAdjacentElement('beforebegin', banner);
        }
